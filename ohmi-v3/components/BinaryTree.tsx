...
function TreeBranch({ node, isAdmin, onEdit, meId, depth = 0 }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      {hasChildren && !collapsed && (
        <>
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'flex-start' }}>
            ...
            {/* Empty slot placeholders (admin only) */}
      ...
      {/* Admin: show empty slots even if no children */}
      {isAdmin && !hasChildren && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ width: 1, height: 20, background: LINE }} />
...
export default function BinaryTree({ portalType, repId }: {
  ...
  // For rep view: build tree from their position down only
  const tree = (() => {
    if (!reps.length) return null
    if (isAdmin) {
      return buildTree(reps, null)
    } else {
      // Rep sees only themselves + downline
    ...
    <div className="main">
      ...
      <div className="topbar">
        <span className="topbar-title">
          {isAdmin ? 'Binary Network Tree' : 'My Network Position'}
        </span>
          ...
          {isAdmin && (
            <div style={{ position: 'relative' }}>
              <input
                ...
                style={{ width: 200, padding: '5px 10px', fontSize: 11 }}
              />
              {searchResults.length > 0 && (
                <div style={{
                  position: 'absolute', top: '100%', right: 0, zIndex: 200,
                  background: '#EDE5CC', border: '1px solid #C4B99A',
                  borderRadius: 3, width: 260, marginTop: 4,
                  boxShadow: '0 8px 24px rgba(26,8,8,0.1)',
                }}>
                  {searchResults.map(r => (
                    <div key={r.id}
                      onClick={() => { setEditing(r); setSearch('') }}
                      style={{
                        padding: '8px 12px', borderBottom: '1px solid #C4B99A',
                        cursor: 'pointer', transition: 'background 0.1s',
                      }}
                      onMouseEnter={e => (e.currentTarget.style.background = '#E5DCBF')}
      ...
      {/* Tree canvas */}
      <div style={{
        flex: 1, overflow: 'auto', padding: '48px 80px 100px',
        background: '#F5EDD6',
        backgroundImage: 'radial-gradient(circle, #C4B99A 1px, transparent 1px)',
        backgroundSize: '32px 32px',
      }}>
        {loading ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300 }}>
            <div style={{ fontSize: 10, color: '#8a6060', letterSpacing: '0.24em', textTransform: 'uppercase' }}>
              Loading network…
            </div>
          </div>
        ) : !tree ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 300, gap: 10 }}>
            <div style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: 26, color: '#C4B99A', fontWeight: 300 }}>
              {isAdmin ? 'No representatives yet' : 'Position not found'}
            </div>
            {isAdmin && (
              <div style={{ fontSize: 10, color: '#8a6060', letterSpacing: '0.14em', textTransform: 'uppercase' }}>
                Add reps from the Network page to build the tree
              </div>
            )}
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            {!isAdmin && (
              <div style={{
                ...
                textTransform: 'uppercase', color: '#C41E4A', marginBottom: 12,
              }}>
                Your Position
              </div>
            )}
            {isAdmin && (
              <div style={{
                fontSize: 9, fontWeight: 700, letterSpacing: '0.24em',
                textTransform: 'uppercase', color: '#8a6060', marginBottom: 12,
              }}>
      ...
      {/* Legend */}
      <div style={{
        display: 'flex', gap: 20, padding: '10px 32px',
        borderTop: '1px solid #C4B99A', background: '#EDE5CC',
        ...
        <span style={{ fontSize: 10, color: '#8a6060', marginLeft: 'auto' }}>
          {isAdmin ? 'Click node to edit · +/− to expand' : '+/− to expand branches'}
        </span>
      </div>
