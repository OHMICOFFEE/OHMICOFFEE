    ...
    <div className="main">
      ...
      <div className="page">
        <div className="pool-meter" style={{marginBottom:20}}>
          ...
          <div className="pool-row"><span className="pool-key">Once-off join fee</span><span className="pool-val">R2,000 — R500 license + R750 coffee + R500 commission + R250 profit</span></div>
          <div className="pool-row"><span className="pool-key">Monthly subscription</span><span className="pool-val">R1,500 — R250 license + R500 coffee + R750 pool</span></div>
          <div className="pool-row"><span className="pool-key">Direct commission</span><span className="pool-val">R500 per signup · paid following Friday</span></div>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,marginBottom:20}}>
          {packages.map(pkg => (
            <div key={pkg.id} style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:4,overflow:'hidden'}}>
              ...
              <div style={{padding:'14px 18px'}}>
                <div style={{fontSize:13,color:'var(--text2)',marginBottom:8}}>{zar(pkg.monthly_fee)}<span style={{fontSize:10,color:'var(--text4)'}}>/month</span></div>
                ...
                {Array.isArray(pkg.features) && pkg.features.map((f:string,i:number) => (
                  <div key={i} style={{fontSize:11,color:'var(--text3)',padding:'3px 0',borderBottom:'1px solid var(--border)',display:'flex',gap:8}}>
                    <span style={{color:'var(--red)'}}>·</span>{f}
                  </div>
                ))}
                <div style={{display:'flex',gap:6,marginTop:12}}>
                  <button className="btn btn-ghost" style={{flex:1}} onClick={()=>openEdit(pkg)}>Edit</button>
                  <button className="btn btn-ghost" style={{flex:1,color:pkg.is_active?'var(--yellow)':'var(--green)'}} onClick={()=>toggleActive(pkg.id,pkg.is_active)}>{pkg.is_active?'Deactivate':'Activate'}</button>
          ...
          {packages.length===0 && <div style={{gridColumn:'1/-1',textAlign:'center',padding:'60px 20px',color:'var(--text4)',fontSize:12}}>No packages yet — add your first one</div>}
        </div>
      </div>
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="grid-2">
                <div className="field span-2"><label>Package Name</label><input value={form.name||''} onChange={e=>set('name',e.target.value)} placeholder="e.g. Standard" /></div>
                <div className="field"><label>Once-off Fee (cents — R2,000 = 200000)</label><input type="number" value={form.once_off_fee||''} onChange={e=>set('once_off_fee',e.target.value)} /></div>
                <div className="field"><label>Monthly Fee (cents — R1,500 = 150000)</label><input type="number" value={form.monthly_fee||''} onChange={e=>set('monthly_fee',e.target.value)} /></div>
                <div className="field"><label>Direct Commission (cents — R500 = 50000)</label><input type="number" value={form.direct_commission||''} onChange={e=>set('direct_commission',e.target.value)} /></div>
                <div className="field"><label>Sort Order</label><input type="number" value={form.sort_order||1} onChange={e=>set('sort_order',e.target.value)} /></div>
                <div className="field span-2"><label>Description</label><input value={form.description||''} onChange={e=>set('description',e.target.value)} /></div>
                <div className="field span-2"><label>Features (one per line)</label><textarea value={form.features||''} onChange={e=>set('features',e.target.value)} rows={5} placeholder={"Network position\nRep portal access\nBinary tree placement"} style={{resize:'vertical'}} /></div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-red" onClick={save} disabled={saving}>{saving?'Saving...':form.id?'Save Changes':'Add Package'}</button>
            </div>
          </div>
        </div>
      )}
