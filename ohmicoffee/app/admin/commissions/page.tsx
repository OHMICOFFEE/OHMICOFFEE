...
type CalcRow = [string, number, (v: number) => void]
...
export default function AdminCommissions() {
  ...
  const calcRows: CalcRow[] = [
    ['Left Leg Volume (R)', leftVol, setLeftVol],
    ['Right Leg Volume (R)', rightVol, setRightVol],
    ['Commission Rate (%)', rate, setRate],
  ]

  return (
    <div className="flex flex-col min-h-screen">
      <AdminTopbar title="Commissions" action={{ label: 'Run Friday Payout', onClick: () => alert('Payout run initiated') }} />
      <div className="flex-1 p-6 overflow-y-auto">
        <div className="bg-[#0c0c0c] border border-white/[0.07] p-5 mb-6">
          ...
          <div className="grid grid-cols-4 gap-4 items-end">
            {calcRows.map(([label, value, setter]) => (
              <div key={label as string}>
                <label className="block text-[10px] tracking-[0.2em] uppercase text-white/30 mb-1">{label as string}</label>
                <input
                  type="number"
          ...
          <table className="w-full">
            ...
            <tbody>
              {commissions.length === 0 && (
                <tr><td colSpan={7} className="text-center py-12 text-white/20 text-[11px] tracking-widest uppercase">No commissions yet</td></tr>
              )}
              {commissions.map(c => (
                <tr key={c.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                  ...
                  <td className="px-4 py-3">
                    <button className="border border-white/[0.12] text-white/40 hover:border-red hover:text-red text-[11px] px-2 py-1 transition-all">
                      {c.status === 'paid' ? 'Receipt' : 'Pay'}
                    </button>
                  </td>
                </tr>
              ))}
